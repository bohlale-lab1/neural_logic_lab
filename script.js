...
async function startCamera() {
    ...
    try {
        ...
        alert('Camera access denied. Please use manual barcode entry or allow camera permissions.');
    }
}
    ...
    if ('BarcodeDetector' in window) {
        try {
            const detector = new BarcodeDetector({ 
                formats: ['code_128', 'ean_13', 'ean_8', 'upc_a', 'upc_e', 'codabar', 'code_39', 'code_93', 'qr_code'] 
            });
        ...
        document.getElementById('scanStatus').innerHTML = '⚠️ Barcode detection not supported in this browser. Use manual entry.';
    }
}
...
async function processScan(barcodeId) {
    console.log("Processing barcode:", barcodeId);
    stopCamera();
    ...
    if (!asset) {
        ...
        if (student) {
            ...
            if (student.accessibility === 'VISUAL_IMPAIRMENT') {
                ...
                speakText(`Access granted for ${student.name}. Exit permitted.`);
            }
        } else {
            decision = 'FLAGGED';
            colour = 'amber';
    ...
    const logEntry = {
        ...
        student_name: student?.name || studentInfo?.student_name,
...
function approveAlert(alertId) {
    ...
    speakText(`Exit approved for ${alert.student_name}`);
}
...
function denyAlert(alertId) {
    ...
    speakText(`Exit denied for ${alert.student_name}`);
}
...
function cancelScan() {
    console.log("Cancel scan called");
    stopCamera();
    showScreen('guardScreen');
...
function manualScan() {
    ...
    console.log("Manual scan:", barcode);
    processScan(barcode);
}
